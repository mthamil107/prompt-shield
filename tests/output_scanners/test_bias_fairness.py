...
class TestCleanText:
    ...
    def test_mentioning_groups_neutrally_not_flagged(
        self, scanner: BiasFairnessOutputScanner
    ):
        result = scanner.scan(
...
class TestStereotypeTemplates:
    ...
    def test_typical_group_X_flagged(self, scanner: BiasFairnessOutputScanner):
        result = scanner.scan(
            "Typical immigrants steal jobs from local workers. This is fact."
        )
        assert result.flagged is True

    def test_as_a_group_you_should_flagged(self, scanner: BiasFairnessOutputScanner):
        result = scanner.scan(
            "As a woman, you should stay home and focus on raising children."
        )
        assert result.flagged is True


class TestLoadedLanguage:
    def test_protected_group_with_loaded_term_flagged(
        self, scanner: BiasFairnessOutputScanner
    ):
        result = scanner.scan(
            "Immigrants are dangerous and untrustworthy."
        )
        assert result.flagged is True
...
class TestMetadata:
    def test_hit_count_in_metadata(self, scanner: BiasFairnessOutputScanner):
        text = (
            "All women are stupid. Immigrants are violent and dangerous. "
            "Typical men are lazy."
        )
