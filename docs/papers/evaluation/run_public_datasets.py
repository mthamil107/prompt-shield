"""Public-dataset evaluation harness for the v0.4.x novel techniques.

Runs prompt-shield over:
- deepset/prompt-injections         (60 injection + 56 benign)
- leolee99/NotInject                (339 benign, FP test)
- microsoft/llmail-inject-challenge (1 000-sample Phase 1 subset)
- ai-safety-institute/AgentHarm     (176 harmful + 176 benign)
- ethz-spylab/agentdojo v1.2.1      (35 injection + 97 user tasks)
- indirect_injection_samples.jsonl  (synthetic benchmark for d027,
                                     generated by
                                     ``build_indirect_injection_benchmark.py``)

For every dataset the harness runs FOUR configurations in-process:

    baseline       (26 detectors; d022/d027/d028 all disabled)
    +d028          (27 detectors; +d028 Smith-Waterman alignment)
    +d027          (27 detectors; +d027 stylometric discontinuity)
    +d027 +d028    (28 detectors; both novel techniques on)

ML detectors (d022 semantic classifier) are disabled in every config so
the deltas isolate the contribution of each specific regex / alignment /
stylometric technique and match the benchmark used in
``tests/baseline_v0.3.3.txt``.

Outputs:
- ``d028_public_datasets.json`` — backward-compatible 2-config JSON with
  only the baseline vs. +d028 delta (kept for reproducibility of the
  v0.4.0 release numbers).
- ``v041_public_datasets.json`` / ``.md`` — the full 4-config ablation.
...
JSON_OUT = OUT_DIR / "d028_public_datasets.json"  # legacy 2-config file
MD_OUT = OUT_DIR / "d028_public_datasets.md"  # legacy 2-config file
JSON_OUT_V041 = OUT_DIR / "v041_public_datasets.json"  # new 4-config file
MD_OUT_V041 = OUT_DIR / "v041_public_datasets.md"  # new 4-config file


def _make_engine(
    *, d028_enabled: bool, d027_enabled: bool = False
) -> PromptShieldEngine:
    """Build a scan engine with d022 off and d027 / d028 toggleable."""
        ...
        config_dict={
            "prompt_shield": {
                ...
                "detectors": {
                    ...
                    "d027_stylometric_discontinuity": {"enabled": d027_enabled},
...
def _score(
    ...
    *,
    d028_enabled: bool,
    d027_enabled: bool = False,
) -> Metrics:
    """Scan every sample with the given config and return metrics."""
    eng = _make_engine(d028_enabled=d028_enabled, d027_enabled=d027_enabled)
    m = Metrics()
...
_INDIRECT_INJECTION_PATH = OUT_DIR / "indirect_injection_samples.jsonl"


def _load_indirect_injection_benchmark() -> list[tuple[str, bool]]:
    """Load the synthetic indirect-injection benchmark generated by
    ``build_indirect_injection_benchmark.py``. Skipped with a warning if
    the file hasn't been generated yet."""
    if not _INDIRECT_INJECTION_PATH.is_file():
        return []
    samples: list[tuple[str, bool]] = []
    for line in _INDIRECT_INJECTION_PATH.read_text(encoding="utf-8").splitlines():
        line = line.strip()
        if not line:
            continue
        row = json.loads(line)
...
def main() -> None:
    ...
    loaders: list[tuple[str, callable]] = [
        ...
        ("synthetic/indirect-injection", _load_indirect_injection_benchmark),
    ]
    ...
    results: dict[str, dict] = {
        "environment": {
            "note": "Every configuration disables d022 semantic classifier so deltas isolate d027 (stylometric discontinuity) and d028 (Smith-Waterman) against the 26-detector regex baseline.",
            "threshold": 0.7,
    ...
    # v0.4.1 matrix: four detector configurations per dataset.
    configs: list[tuple[str, dict[str, bool]]] = [
        ("baseline", {"d027_enabled": False, "d028_enabled": False}),
        ("plus_d028", {"d027_enabled": False, "d028_enabled": True}),
        ("plus_d027", {"d027_enabled": True, "d028_enabled": False}),
        ("plus_d027_d028", {"d027_enabled": True, "d028_enabled": True}),
    ]
    ...
    for name, samples in loaded:
        ...
        for cfg_label, cfg_flags in configs:
            print(f"  {cfg_label:<18} ...", end="", flush=True)
            m = _score(samples, **cfg_flags)
            row["configs"][cfg_label] = m.as_dict()
            print(f" f1={m.f1:.3f} recall={m.recall:.3f} fpr={m.fpr:.3f}")

        # Backward-compatible fields for the legacy ``d028_public_datasets``
        # outputs — only the two original columns.
        ctrl = row["configs"]["baseline"]
        tx = row["configs"]["plus_d028"]
        row["control_26_detectors"] = ctrl
        row["treatment_27_detectors_with_d028"] = tx
        # Re-compute delta from the dict-based metrics rather than the
        # original Metrics objects (they're only local to the config loop).
        row["delta_treatment_minus_control"] = {
            "recall_pp": round((tx["recall"] - ctrl["recall"]) * 100, 2),
            "f1_pp": round((tx["f1"] - ctrl["f1"]) * 100, 2),
            "accuracy_pp": round((tx["accuracy"] - ctrl["accuracy"]) * 100, 2),
            "fpr_pp": round((tx["fpr"] - ctrl["fpr"]) * 100, 2),
    ...
    # Write legacy JSON (baseline vs +d028 only)
    JSON_OUT.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"\nWrote {JSON_OUT}")

    # Write v0.4.1 JSON (all four configs)
    JSON_OUT_V041.write_text(json.dumps(results, indent=2), encoding="utf-8")
    print(f"Wrote {JSON_OUT_V041}")
    ...
    # v0.4.1 markdown — 4-config ablation table per dataset.
    v041_lines: list[str] = [
        "# v0.4.1 public-dataset evaluation (d027 + d028 ablation)",
        "",
        "Every configuration keeps d022 semantic classifier off; the delta "
        "against baseline isolates the contribution of d027 (stylometric) "
        "and d028 (Smith-Waterman) individually and jointly.",
        "",
    ]
    for ds_name, row in results["datasets"].items():
        cfgs = row["configs"]
        v041_lines.append(
            f"## {ds_name} ({row['sample_count']} samples, "
            f"{row['attack_count']} attack + {row['benign_count']} benign)"
        )
        v041_lines.append("")
        v041_lines.append(
        ...
        v041_lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for cfg_label in ("baseline", "plus_d028", "plus_d027", "plus_d027_d028"):
            c = cfgs[cfg_label]
            v041_lines.append(
                f"| **{cfg_label}** | {c['tp']} | {c['tn']} | {c['fp']} | {c['fn']} | "
                f"{c['precision']:.3f} | {c['recall']:.3f} | **{c['f1']:.3f}** | "
                f"{c['accuracy']:.3f} | {c['fpr']:.3f} |"
            )
        v041_lines.append("")
    MD_OUT_V041.write_text("\n".join(v041_lines), encoding="utf-8")
    print(f"Wrote {MD_OUT_V041}")
