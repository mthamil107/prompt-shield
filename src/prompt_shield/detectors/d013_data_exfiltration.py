...
class DataExfiltrationDetector(BaseDetector):
    ...
    description: str = "Detects attempts to make the AI send data to external destinations"
    ...
    _patterns: ClassVar[list[tuple[str, str]]] = [
        (
            r"send\s+(?:(?:the|your|all|this|my)\s+)?(?:response|data|output|conversation|info(?:rmation)?|results?|content)\s+to\s+\S+",
            "Send data to external destination",
        ),
        ...
        (
            r"(?:email|mail|send)\s+(?:the\s+)?(?:results?|data|output|info(?:rmation)?|conversation)\s+to\s+\S+@\S+",
            "Email exfiltration",
        ),
        (
            r"forward\s+(?:all\s+)?(?:data|info(?:rmation)?|results?)\s+to\s+\S+@\S+",
            "Forward to email",
        ),
        ...
        (
            ...
            "URL with IP address in context",
        ),
        (r"curl\s+", "Curl command"),
        (r"fetch\s*\(\s*[\"']https?://", "Fetch API call"),
        (r"XMLHttpRequest", "XHR request"),
        (
            r"(?:send|transmit|relay|pipe|post)\s+(?:(?:the|your|all|this)\s+)?(?:data|output|response|conversation|info(?:rmation)?)\s+(?:to|over\s+to)\s+(?:https?://|ftp://)\S+",
            "Send data to URL",
        ),
        (
            r"(?:steal|collect|harvest|scrape)\s+(?:the\s+)?(?:data|info(?:rmation)?|conversation|credentials?)",
            "Data stealing intent",
        ),
    ]
