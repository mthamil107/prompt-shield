...
class RoleHijackDetector(BaseDetector):
    ...
    tags: ClassVar[list[str]] = ["direct_injection", "jailbreak"]
    version: str = "1.0.0"
    ...
    _patterns: ClassVar[list[tuple[str, str]]] = [
        (
            r"\byou\s+are\s+now\s+(?:a\s+)?(?:DAN|Do\s+Anything\s+Now)\b",
        ...
        (
            r"\b(?:pretend|imagine|suppose)\s+(?:that\s+)?you\s+are\b",
            "Persona impersonation request",
        ),
        (
            r"\bfrom\s+(?:now|this\s+point)\s+on\s+you\s+(?:will|shall|must|should)\b",
            "Attempt to redefine model behavior",
        ),
        (r"\byou\s+are\s+no\s+longer\b", "Attempt to discard current role"),
        (
            r"\b(?:switch|change|transition)\s+(?:(?:in)?to|over\s+to)\s+\S+\s+mode\b",
            "Mode switching attempt",
        ),
        (
            r"\b(?:enter|activate|enable|turn\s+on)\s+(?:the\s+)?developer\s+mode\b",
            "Developer mode jailbreak attempt",
        ),
        (
            r"\b(?:enable|activate|enter|turn\s+on)\s+(?:the\s+)?jailbreak(?:\s+mode)?\b",
            "Explicit jailbreak mode request",
        ),
        (
            r"\byour\s+(?:new\s+)?name\s+is\s+(?:now\s+)?\b",
            "Attempt to rename the model",
        ),
        (r"\bforget\s+(?:that\s+)?you\s+are\b", "Attempt to erase model identity"),
        (
            r"\byou\s+(?:must|should|have\s+to|need\s+to)\s+(?:now\s+)?act\s+as\b",
            "Forceful role reassignment",
        ),
        (
            r"\b(?:assume|adopt|take\s+on)\s+the\s+(?:role|persona|identity|character)\s+of\b",
            "Role assumption instruction",
        ),
    ]
